        ...
        $Connect = @fsockopen($IP, "80", $errno, $errstr, 1);
            if($Connect){
                ...
                for($a=0;$a<count($buffer);$a++){
                    ...
                    if (!$this->if_exist_check($PIN, $DateTime) ) {         
                        $Status2 = $Status;
                        $PIN2 = $PIN;
                        $DateTime2 = $DateTime;
                        $Verified2 = $Verified;
                ...
                for ($dapet=0;$dapet<1;$dapet++) {
                    $datapulang3 = $datapulang[0]['jam_masuk'];
                    $tgl      = date('H:i', $datapulang3);
                    $waktu    = date('H:i', strtotime($datapulang3.'+60 second'));


                    $dapet++;
                    var_dump($tgl); var_dump('>=');
                    var_dump($waktu);
                }     
                        switch ($Status2) {
                            case '0':
                                ...
                                $fields = array(
                                    ...
                                    'device'    => '33026',
                                    'number'    => '085883518288',
                                    'message'   => 'Anak anda Fakhri keluar dari kelas',
                                    'send_at'   => date()
                                );
